...
func init() {
	...
	// Register OpenShift and Prometheus Operator types
...
func main() {
	...
	// http/2 stays disabled by default against the Stream Cancellation
	// and Rapid Reset CVEs (GHSA-qppj-fm5r-hxr3, GHSA-4374-p667-p6c8).
	...
	// The metrics endpoint itself is enabled in
	...
	if secureMetrics {
		// Inert without the auth/reader RBAC manifests listed in
		// config/rbac/kustomization.yaml.
		metricsServerOptions.FilterProvider = filters.WithAuthenticationAndAuthorization
	}

	// Unset: controller-runtime serves metrics with a self-signed cert.
	// cert-manager serving would need the [METRICS-WITH-CERTS] and
	// [PROMETHEUS-WITH-CERTS] kustomize markers uncommented.
	...
	mgr, err := ctrl.NewManager(ctrl.GetConfigOrDie(), ctrl.Options{
		...
		// Only safe if the process exits immediately when the manager
		// stops — unsafe with any post-manager cleanup.
...
// namespaceFromEnv returns the operator's own namespace, resolved from
// the downward-API POD_NAMESPACE env var with a fallback to the
