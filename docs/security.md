...
Secret read in a namespace is control of the Nomad clusters in that
namespace (the [threat model](threat-model/README.md) develops this).
The tenant boundary is therefore the namespace.
...
Two pieces of supply-chain hardening are not in this release:

- **Container images are not signed.** No cosign signatures are
  attached, so signature verification is not available against the
  upstream-published images.
- **No SBOM is attached to releases.** Software composition analysis
  runs against the image itself with the consumer's tooling.

For environments that require signed images, SBOMs, or provenance
attestation:

1. Pull the upstream image
   (`quay.io/benjamin_holmes/nomad-enterprise-operator:v<version>`).
2. Scan with your preferred scanner (Trivy, Snyk, Grype).
3. Re-tag and push to your internal registry; sign per your
   organisation's policy (cosign, notation).
4. Deploy from the internal-registry tag; override the operator image
   in the deployment manifest.

The operator runs from any image with the same entrypoint and binary
contract, so this workflow needs no code changes.

### Bootstrap token Secret lifecycle

The ACL bootstrap token Secret (`<cluster>-acl-bootstrap`) has no
ownerReference to its NomadCluster. If it did, Kubernetes
...
!!! warning
    If the finalizer never completes (operator uninstalled before the
    cluster was deleted, namespace force-deleted with the finalizer
    stripped), the bootstrap Secret is orphaned and nothing garbage
    collects it. It contains a Nomad management token. The operator
    does not scan for orphans.

To sweep for orphans:

1. List bootstrap Secrets and live clusters:
...
2. Delete any Secret whose `nomad.hashicorp.com/cluster` label names a
   cluster that no longer exists. The token it holds died with the
   cluster.

## Operator RBAC

The threat model's primary mitigation presumes the bundled ClusterRole
is minimal. Every permission in the generated role
